...
export function AppProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (token) {
      getProfile(); // Fetch profile directly; set isAuthenticated based on response
    } else {
      setIsAuthenticated(false); // Ensure false if no token
    }
    fetchProducts();
  }, []);
  ...
  const getProfile = async () => {
    ...
    try {
      ...
      setUser(customer);
      setIsAuthenticated(true); // Set true on success
      setError(null);
      return customer;
    } catch (err) {
      setError(err.response?.data?.error || "Failed to fetch profile");
      setIsAuthenticated(false); // Reset on failure (e.g., invalid token)
